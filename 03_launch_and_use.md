# 03 · 四种方式唤醒 Claude Code — 找到适合你的工作流
...
```bash
cca   # 等价于：cd ~/claude/ && claude --dangerously-skip-permissions
```
...
Claude Code 有官方的 VS Code 插件，安装后可以让它直接感知你当前打开的文件和选中的代码，体验更流畅。
...
在 VS Code 的**集成终端**（`Ctrl+\`` 打开）里执行 `cc` 即可：
